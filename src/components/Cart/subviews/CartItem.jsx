...
import { setDecreseItemQty, setIncreseItemQty, setRemoveItemFromCart } from '../../../process/app/CartSlice';

const CartItem = ({ item: { id, title, text, img, color, shadow, price, cartQuantity } }) => {
  const dispatch = useDispatch();
  const itemFinalPrice = price * cartQuantity;
  ...
  const onIcreaseItemQty = () => {
    dispatch(setIncreseItemQty({ id, title, text, img, color, shadow, price, cartQuantity }));
  };
  const onDecreseItemQty = () => {
    dispatch(setDecreseItemQty({ id, title, text, img, color, shadow, price, cartQuantity }));
  };
      ...
      <div className="flex items-center justify-between w-full px-5 py-4">
        <div className="flex items-center gap-10">
          ...
          <div className="grid items-center gap-4">
            ...
            <div className="flex items-center w-full">
              <div className="flex flex-start items-center justify-evenly w-full">
                <button
                  onClick={onDecreseItemQty}
                  type="button"
        ...
        <div className="grid items-center gap-5">
          <div className="flex items-center justify-evenly">
            <h1 className="text-lg lg:text-base text-slate-900 font-medium">${itemFinalPrice}</h1>
          </div>
